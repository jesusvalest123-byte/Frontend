...
function EliminarActividad({ actividad, onBack, onEliminado }) {
  ...
  const handleDelete = async () => {
    setCargando(true);
    try {
      await axios.delete(`http://localhost:8080/api/actividades/${actividad.idActividad}`);
      toast.success(`🗑️ Actividad "${actividad.nombreActividad}" eliminada con éxito`, {
        position: "top-right",
        autoClose: 3000,
      });
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md max-w-md mx-auto text-center relative">
      <ToastContainer />
      <h2 className="text-2xl font-bold text-red-600 mb-6">Eliminar Actividad</h2>
      <p className="text-gray-700 mb-6">
        ¿Deseas eliminar la actividad{" "}
        <span className="font-bold">{actividad.nombreActividad}</span>?
      </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full text-center">
            ...
            <p className="mb-6 text-gray-700">
              ...
              ¿Estás seguro que quieres eliminar la actividad{" "}
              <span className="font-bold">{actividad.nombreActividad}</span>?
            </p>
            <div className="flex justify-center gap-4">
              <button
